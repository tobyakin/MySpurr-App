<script setup>
    ...
    const props = defineProps(['messageList', 'filter'])
...
<template>
  <div>
    <article id="message" class="p-[1.25rem] border-y border-[#F3F3F3] cursor-pointer" v-for="message in messageList" :key="message" @click="showChatPane(message)">
          ...
          <div class="messageDetail pl-[1rem]">
            <h3 class="messageTitle font-Satoshi500 text-[#000] leading-[1.5rem] text-[0.8rem] !mb-[0.3rem] capitalize">{{ message.attributes.subject }}</h3>
